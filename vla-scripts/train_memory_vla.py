...
from prismatic.models import load, load_vla, load_or_initialize_vlm
from prismatic.models.vlas.memory_vla_v2 import MemoryVLA_V2
...
class TrainConfig:
    ...
    # )
    vla: VLAConfig = field(
        # default_factory=VLAConfig.get_choice_class(VLARegistry.DINOSIGLIP_224PX_MX_MIMICLABS.vla_id)
        # default_factory=VLAConfig.get_choice_class(VLARegistry.FREEZE_SIGLIP_224PX_MX_MIMICLABS.vla_id)
        default_factory=VLAConfig.get_choice_class(VLARegistry.FREEZE_SIGLIP_224PX_MX_MIMICLABS_BINNING.vla_id)
    )
    ...
    save_interval: int = 250 # 2500                                       # Interval for saving checkpoints (in steps)
    ...
    # Model Parameters
    frame_stack: int = 10                                           # Number of additional sequential frames to stack (max context_length-1)
    control_delta: bool = True                                      # Whether to use delta control for actions
    context_length: int = 16                                        # Context length for the CausalContextTransformer
    causal_num_heads: int = 8                                       # Number of attention heads in CausalContextTransformer
    causal_num_layers: int = 4                                      # Number of transformer layers in CausalContextTransformer
    causal_dropout: float = 0.0                                     # Dropout probability in CausalContextTransformer
    action_dim: int = 7                                             # Dimension of action space

    # Tracking Parameters
    trackers: Tuple[str, ...] = ("jsonl", "wandb")                  # Trackers to initialize (if W&B, add config!)
    wandb_project: str = "openvla"                                  # Name of W&B project to log to (use default!)
    wandb_entity: str = "vaibhavsaxena11-georgia-tech"              # Name of entity to log under
...
def train(cfg: TrainConfig) -> None:
    ...
    cfg.run_id = (
        ...
        # f"{vla_id}+n{cfg.vla.expected_world_size // 8}+b{cfg.global_batch_size}+x{cfg.seed}"
        f"{vla_id}+n{cfg.vla.expected_world_size // 8}+b{cfg.global_batch_size}+x{cfg.seed}+frame{cfg.frame_stack}"
    ...
    if cfg.pretrained_checkpoint is not None:
        ...
        if cfg.is_resume:
            step_match = re.search(r"step-(\d+)-", str(cfg.pretrained_checkpoint))
            epoch_match = re.search(r"epoch-(\d+)-", str(cfg.pretrained_checkpoint))
        ...
        overwatch.info("Loading MemoryVLA_V2 Checkpoint")
        ...
        vla = MemoryVLA_V2(
            ...
            action_tokenizer=None,  # Will be set after dataset loading
            context_length=cfg.context_length,
            causal_num_heads=cfg.causal_num_heads,
            causal_num_layers=cfg.causal_num_layers,
            causal_dropout=cfg.causal_dropout,
        )
        
        # Load the state dict
        vla.load_state_dict(checkpoint, strict=False)
        del vlm, checkpoint

    else:
        # Load base VLM (or initialize fresh if checkpoint not found)
        vlm = load_or_initialize_vlm(cfg.vla.base_vlm, hf_token=hf_token, load_for_training=True)
        
        overwatch.info("Creating MemoryVLA_V2 from Base VLM")
        vla = MemoryVLA_V2(
            model_id=vlm.model_id,
            vision_backbone=vlm.vision_backbone,
            llm_backbone=vlm.llm_backbone,
            enable_mixed_precision_training=vlm.enable_mixed_precision_training,
            arch_specifier=vlm.arch_specifier,
            norm_stats={},  # Will be populated after dataset loading
            action_tokenizer=None,  # Will be set after dataset loading
            context_length=cfg.context_length,
            causal_num_heads=cfg.causal_num_heads,
            causal_num_layers=cfg.causal_num_layers,
    ...
    vla_dataset, action_tokenizer, collator = get_vla_dataset_and_collator(
        ...
        data_format=cfg.vla.data_format,
        shuffle_buffer_size=cfg.vla.shuffle_buffer_size,
        image_aug=cfg.image_aug,
        train=True,
        episodic=False,
        frame_stack=cfg.frame_stack,
        action_keys=("actions",) if cfg.control_delta else ("actions_abs",),
    )
    ...
    # Update VLA with action tokenizer and norm stats
    vla.action_tokenizer = action_tokenizer
    vla.norm_stats = vla_dataset.dataset_statistics
    
    # # Now that we have action_dim, initialize the action head if not already done
    # if vla.action_head is None:
    #     vla.action_head = ActionPredictionHead(
    #         token_dim=vla.cog_token_size,
    ...
    train_strategy.run_memory_vla_training(
        vla_dataset,
        collator,
        action_tokenizer,
        metrics,
